...
export function ThemeToggle({ isDropDown = false }: { isDropDown?: boolean }) {
  ...
  if (!mounted) {
    return <Loader />; // or whatever fallback you prefer
  }
  if (isDropDown) {
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="lg"
            className="flex justify-start mb-2 px-2 h-9 gap-2 w-full whitespace-nowrap rounded-md text-sm font-base transition-colors disabled:pointer-events-none disabled:opacity-50  focus:outline-none"
          >
            <Sun
              className={cn(
